...
export default function BankMessages() {
  ...
  const review = row => {
    ...
    setSelected(row); setError(''); setForm({ amount:suggestions.amount||'', description:suggestions.description||'', wallet_id:wallets[0]?.id || '', date:suggestions.date||dateInput() })
  }
  const record = e => { e.preventDefault(); act(async () => {
    await api(`/api/bank-messages/${selected.id}/record`, { method:'POST', ...jsonBody({ ...form, type:'expense', amount:Number(form.amount), wallet_id:Number(form.wallet_id), date:saveDate(form.date), notes:`Imported from ${selected.bank} message` }) })
  ...
  return <div className="stack gap-18">
    ...
    <section className="panel glass"><div className="section-row"><h3>Pending review ({rows.length})</h3><button className="button ghost" disabled={busy} onClick={() => act(load)}>Refresh</button></div>{loading ? <p role="status">Loading messages...</p> : rows.length === 0 ? <p className="muted">No pending messages.</p> : rows.map(row => <div className="bank-message" key={row.id}><strong>{row.bank}</strong><small>{showTime(row.created_at)} Kuwait</small><p>{row.message}</p>{row.possible_duplicate&&<p className="form-error">Possible duplicate. Check existing messages and transactions before recording.</p>}<div className="button-row"><button className="button primary" onClick={() => review(row)}>Review expense</button><button className="icon-button" aria-label="Remove message" onClick={async () => { if (!await confirm('Remove this message?')) return; act(async () => { await api(`/api/bank-messages/${row.id}`, {method:'DELETE'}); await load() }) }}><Trash2 size={18}/></button></div></div>)}</section>
