...
export default function Settings(){
  ...
  const save=async e=>{e.preventDefault();try{await configureReminder(form);const updated=await api('/api/settings',{method:'PUT',...jsonBody(form)});setSettings(updated);if(!isNativeApp()&&form.reminders_enabled&&'Notification' in window&&window.Notification.permission==='default'){const permission=await window.Notification.requestPermission();notify(permission==='granted'?'Settings saved. Browser reminders work while Budgetly is open.':'Settings saved. Device notification permission is blocked.')}else notify('Settings saved')}catch(err){notify(err.message,'error')}}
  ...
  const toggleWallpaper=async enabled=>{setAppearanceBusy(true);try{const updated=await api('/api/settings',{method:'PUT',...jsonBody({...settings,wallpaper_enabled:enabled})});setSettings(updated);notify(enabled?'Wallpaper enabled':'Wallpaper disabled')}catch(err){notify(err.message,'error')}finally{setAppearanceBusy(false)}}
