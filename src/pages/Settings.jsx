...
import { Camera, Download, Fingerprint, Image, Mountain, Pencil, Plus, RefreshCcw, Save, ShieldCheck, Trash2, Upload, UserRound, X } from 'lucide-react'
...
<section className="panel glass"><div className="panel-head"><div><p className="eyebrow">Quick access</p><h3>Biometric sign-in</h3></div><Fingerprint className="muted-icon"/></div><p className="muted">Use Face ID, Touch ID, Windows Hello, or your device passkey to unlock this device faster. Your password remains available as a fallback.</p><div className="stack gap-12"><DeviceSignInPreference disabled={passkeyBusy} onChange={setPasswordOnly}/>{!passwordOnly && <label className="field"><span>Confirm password to register a passkey</span><PasswordInput autoComplete="current-password" value={passkeyPassword} onChange={e=>setPasskeyPassword(e.target.value)}/></label>}{biometric && <button className="button ghost" disabled={passkeyBusy} onClick={disableDeviceSignIn}>Revoke all passkeys</button>}{<button className="button primary" disabled={!biometricSupported() || passkeyBusy || passkeyPassword.length < 8} onClick={enableBiometric}><Fingerprint/>{passkeyBusy ? 'Registering...' : biometric ? 'Register another passkey' : 'Enable biometric sign-in'}</button>}</div>{!biometricSupported()&&<small className="muted">{passwordOnly ? 'Password-only sign-in is enabled for this device.' : 'Passkeys require a supported browser and HTTPS.'}</small>}</section>
    ...
    <section className="panel glass"><div className="panel-head"><div><p className="eyebrow">Fresh start</p><h3>Reset workspace</h3></div><RefreshCcw className="muted-icon"/></div><p className="muted">Clear budget activity and extra wallets. Your default Main Wallet stays available at zero, and categories are preserved.</p><button className="button ghost full" onClick={async()=>{try{setClearPreview(await api('/api/workspace/clear-preview'));setClearOpen(true)}catch(e){notify(e.message,'error')}}}><RefreshCcw/>Reset workspace</button>{recovery.length>0&&<div className="recovery-list"><small className="muted">Recent recovery copies</small>{recovery.slice(0,3).map(item=><button key={item.id} className="recovery-row" onClick={()=>downloadRecovery(item.id)}><span><strong>{item.reason}</strong><small>{showTime(item.created_at)}</small></span><Download size={15}/></button>)}</div>}</section>
