...
import { configureReminder, isNativeApp } from '../lib/deviceNotifications'
import { readNotificationSettings, saveNotificationSettings, stripNotificationSettings } from '../lib/notificationSettings'
...
export default function Settings(){
  ...
  const [notificationForm,setNotificationForm]=useState(() => readNotificationSettings(settings, user?.id))
  ...
  const [passwordOnly,setPasswordOnly]=useState(passwordOnlyEnabled)
  useEffect(() => () => cancelBiometric(), [])
  useEffect(() => { api('/api/passkeys').then(r => setBiometric(r.enabled)).catch(e => notify(e.message, 'error')) }, [notify])
  const inputRef=useRef(null)
  const profileRef=useRef(null)
  const wallpaperRef=useRef(null)
  useEffect(()=>setForm(settings),[settings])
  useEffect(()=>setNotificationForm(readNotificationSettings(settings, user?.id)),[settings,user?.id])
  ...
  const toggleWallpaper=async enabled=>{setAppearanceBusy(true);try{const updated=await api('/api/settings',{method:'PUT',...jsonBody({...stripNotificationSettings(settings),wallpaper_enabled:enabled})});setSettings({...settings,...updated});notify(enabled?'Wallpaper enabled':'Wallpaper disabled')}catch(err){notify(err.message,'error')}finally{setAppearanceBusy(false)}}
  ...
  const enableBiometric=async()=>{if(passkeyBusy)return;setPasskeyBusy(true);try{await setupBiometric(user.email,passkeyPassword);setBiometric(true);setPasskeyPassword('');notify('Passkey registered')}catch(err){notify(err.name==='NotAllowedError'?'Passkey setup cancelled':err.message,'error')}finally{setPasskeyBusy(false)}}
  ...
  return <div className="settings-grid">
    ...
    <SettingsSection title="Display & locale" className=""><form onSubmit={save} className="stack gap-16"><label className="field"><span>Budget name</span><input value={form.display_name||''} onChange={e=>setForm({...form,display_name:e.target.value})}/></label><div className="form-grid two"><label className="field"><span>Currency</span><input value={form.currency||'KWD'} maxLength="6" onChange={e=>setForm({...form,currency:e.target.value.toUpperCase()})}/></label><label className="field"><span>Week starts on</span><select value={form.week_starts_on||'sunday'} onChange={e=>setForm({...form,week_starts_on:e.target.value})}><option value="sunday">Sunday</option><option value="monday">Monday</option></select></label></div><label className="check-row"><input type="checkbox" checked={!!form.compact_numbers} onChange={e=>setForm({...form,compact_numbers:e.target.checked})}/><span>Use compact large numbers where possible</span></label><button className="button primary self-start"><Save/>Save preferences</button></form></SettingsSection>
    <SettingsSection title="Notifications" className=""><p className="muted">Configure this {isNativeApp() ? 'Android device' : 'browser'} separately. Browser reminders work while Budgetly is open; Android reminders depend on device notification and battery settings.</p><form onSubmit={saveNotifications} className="stack gap-16"><label className="check-row"><input type="checkbox" checked={!!notificationForm.reminders_enabled} onChange={e => setNotificationForm({...notificationForm, reminders_enabled:e.target.checked})}/><span>Transaction reminders</span></label>{notificationForm.reminders_enabled && <><label className="field"><span>Repeat every</span><select value={notificationForm.reminder_interval_hours || 4} onChange={e=>setNotificationForm({...notificationForm,reminder_interval_hours:Number(e.target.value)})}>{[1,2,3,4,6,8,12,24].map(hours=><option key={hours} value={hours}>{hours===24?'Once a day':`${hours} hour${hours===1?'':'s'}`}</option>)}</select></label><label className="field"><span>First reminder (Kuwait)</span><input required type="time" value={notificationForm.reminder_time || '20:00'} onChange={e => setNotificationForm({...notificationForm, reminder_time:e.target.value})}/></label><NotificationPreferences form={notificationForm} setForm={setNotificationForm}/></>}<button className="button primary self-start"><Save/><span>Save notifications</span></button></form></SettingsSection>
...
<SettingsSection title="Biometric sign-in" className=""><p className="muted">Use Face ID, Touch ID, Windows Hello, or your device passkey to unlock this device faster. Your password remains available as a fallback.</p><div className="stack gap-12"><DeviceSignInPreference disabled={passkeyBusy} onChange={setPasswordOnly}/>{!passwordOnly && <label className="field"><span>Confirm password to register a passkey</span><PasswordInput autoComplete="current-password" value={passkeyPassword} onChange={e=>setPasskeyPassword(e.target.value)}/></label>}{biometric && <button className="button ghost" disabled={passkeyBusy} onClick={disableDeviceSignIn}>Revoke all passkeys</button>}{<button className="button primary" disabled={!biometricSupported() || passkeyBusy || passkeyPassword.length < 8} onClick={enableBiometric}><Fingerprint/>{passkeyBusy ? 'Registering...' : biometric ? 'Register another passkey' : 'Enable biometric sign-in'}</button>}</div>{!biometricSupported()&&<small className="muted">{passwordOnly ? 'Password-only sign-in is enabled for this device.' : 'Passkeys require a supported browser and HTTPS.'}</small>}</SettingsSection>
    <SettingsSection title="Delete account" className="danger-zone"><p className="muted">Permanently removes your account, wallets, transactions, notes, feedback, and settings.</p><button className="button danger" onClick={deleteAccount}><Trash2/>Delete my account</button></SettingsSection>

    <SettingsSection title="Workspace wallpaper" className="span-2-settings appearance-panel"><div className="wallpaper-settings"><div className={`wallpaper-preview ${appearance.wallpaper_image?'has-image':''}`} style={appearance.wallpaper_image?{backgroundImage:`url(${JSON.stringify(appearance.wallpaper_image)})`}:{}}>{!appearance.wallpaper_image&&<><Image/><span>No wallpaper selected</span></>}</div><div className="stack gap-10"><label className="check-row"><input type="checkbox" role="switch" disabled={appearanceBusy || !appearance.wallpaper_image} checked={settings.wallpaper_enabled !== false} onChange={e=>toggleWallpaper(e.target.checked)}/><span>Enable wallpaper</span></label><div className="button-row"><button className="button primary small" disabled={appearanceBusy} onClick={()=>wallpaperRef.current?.click()}><Upload/>Choose wallpaper</button>{appearance.wallpaper_image&&<button className="button ghost small" disabled={appearanceBusy} onClick={()=>clearAppearance('wallpaper_image')}><Trash2/>Remove wallpaper</button>}</div><small className="appearance-hint">PNG, JPEG, or WebP · maximum 2.5 MB</small></div><input ref={wallpaperRef} hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={pickWallpaper}/></div></SettingsSection>

    <SettingsSection title="Categories" className="span-2-settings"><div className="panel-head"><div></div><button className="button ghost small" onClick={()=>{setEditingCat(null);setCat({name:'',kind:'expense',icon:'circle',color:'#0a4173'});setCatOpen(true)}}><Plus/>Add category</button></div><div className="category-settings">{cats.map(c=><div key={c.id}><span><i style={{background:c.color}}/>{c.name}<small>{c.kind}</small></span><div className="button-row"><button className="row-icon" aria-label={`Edit ${c.name}`} onClick={()=>editCat(c)}><Pencil/></button><button className="row-icon danger" onClick={()=>removeCat(c)}><Trash2/></button></div></div>)}</div></SettingsSection>

    <SettingsSection title="Backup & restore" className=""><p className="muted">Export a JSON snapshot of your workspace. Other users and passwords are never included.</p><div className="stack gap-10"><button className="button ghost full" onClick={exportBackup}><Download/>Export backup</button><button className="button ghost full" onClick={()=>inputRef.current?.click()}><Upload/>Restore backup</button><input ref={inputRef} hidden type="file" accept="application/json,.json" onChange={restore}/></div></SettingsSection>

    <SettingsSection title="Reset workspace" className=""><p className="muted">Clear budget activity and extra wallets. Your default Main Wallet stays available at zero, and categories are preserved.</p><button className="button ghost full" onClick={async()=>{try{setClearPreview(await api('/api/workspace/clear-preview'));setClearOpen(true)}catch(e){notify(e.message,'error')}}}><RefreshCcw/>Reset workspace</button></SettingsSection>

    <Modal open={catOpen} onClose={()=>setCatOpen(false)} title={editingCat?'Edit category':'Add category'}><form onSubmit={saveCat} className="stack gap-16"><label className="field"><span>Name</span><input required value={cat.name} onChange={e=>setCat({...cat,name:e.target.value})}/></label><div className="form-grid two"><label className="field"><span>Type</span><select value={cat.kind} onChange={e=>setCat({...cat,kind:e.target.value})}><option value="expense">Expense</option><option value="income">Income</option></select></label><label className="field"><span>Accent</span><input type="color" value={cat.color} onChange={e=>setCat({...cat,color:e.target.value})}/></label></div><div className="modal-actions"><button type="button" className="button ghost" onClick={()=>setCatOpen(false)}>Cancel</button><button className="button primary">{editingCat?'Save category':'Add category'}</button></div></form></Modal>
    <Modal open={clearOpen} onClose={()=>setClearOpen(false)} title="Reset workspace"><form onSubmit={clearWorkspace} className="stack gap-16"><p className="muted">This only affects your account. Type <strong>CLEAR</strong> to confirm. Categories are never removed by this action. Recovery copy will be available in Settings.</p>{clearPreview&&<div className="muted"><p>{clearPreview.transactions} transactions, {clearPreview.budgets} budgets, {clearPreview.goals} goals and {clearPreview.debts} debts will be cleared. Main Wallet will be zeroed; extra wallets will be removed. {clearPreview.categories_preserved} categories will remain.</p>{clearScope==='workspace'&&<p>{clearPreview.notes} notes will also be removed.</p>}<button type="button" className="button ghost" onClick={exportBackup}><Download size={16}/>Export before clearing</button></div>}<label className="field"><span>What should be cleared?</span><select value={clearScope} onChange={e=>setClearScope(e.target.value)}><option value="budget">Budget data and extra wallets</option><option value="workspace">Budget data, extra wallets, and Notes</option></select></label><label className="field"><span>Confirmation</span><input required value={clearConfirm} onChange={e=>setClearConfirm(e.target.value)} placeholder="Type CLEAR" autoComplete="off"/></label><div className="modal-actions"><button type="button" className="button ghost" onClick={()=>setClearOpen(false)}>Cancel</button><button className="button danger" disabled={clearConfirm !== 'CLEAR'}>Reset workspace</button></div></form></Modal>
  </div>
}
