...
export default function Settings(){
  ...
  return <div className="settings-grid">
    ...
    <SettingsSection title={isNativeApp() ? 'Android notifications' : 'Browser notifications'} className=""><form onSubmit={saveNotifications} className="stack gap-16"><label className="check-row"><input type="checkbox" checked={!!notificationForm.reminders_enabled} onChange={e => setNotificationForm({...notificationForm, reminders_enabled:e.target.checked})}/><span>Transaction reminders</span></label>{notificationForm.reminders_enabled && <><label className="field"><span>Repeat every</span><select value={notificationForm.reminder_interval_hours || 4} onChange={e=>setNotificationForm({...notificationForm,reminder_interval_hours:Number(e.target.value)})}>{[1,2,3,4,6,8,12,24].map(hours=><option key={hours} value={hours}>{hours===24?'Once a day':`${hours} hour${hours===1?'':'s'}`}</option>)}</select></label><label className="field"><span>First reminder (Kuwait)</span><input required type="time" value={notificationForm.reminder_time || '20:00'} onChange={e => setNotificationForm({...notificationForm, reminder_time:e.target.value})}/></label><NotificationPreferences form={notificationForm} setForm={setNotificationForm}/></>}<button className="button primary self-start"><Save/><span>Save {isNativeApp() ? 'Android' : 'browser'} notifications</span></button></form></SettingsSection>
