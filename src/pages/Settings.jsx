...
export default function Settings(){
  ...
  return <div className="settings-grid">
    ...
    <section className="panel glass"><div className="panel-head"><div><p className="eyebrow">Fresh start</p><h3>Clear workspace</h3></div><RefreshCcw className="muted-icon"/></div><p className="muted">Remove this account's budget data. A recovery copy is saved automatically before anything is cleared.</p><button className="button ghost full" onClick={()=>setClearOpen(true)}><RefreshCcw/>Clear my workspace</button>{recovery.length>0&&<div className="recovery-list"><small className="muted">Recent recovery copies</small>{recovery.slice(0,3).map(item=><button key={item.id} className="recovery-row" onClick={()=>downloadRecovery(item.id)}><span><strong>{item.reason}</strong><small>{new Date(item.created_at).toLocaleString()}</small></span><Download size={15}/></button>)}</div>}</section>
    ...
    <Modal open={clearOpen} onClose={()=>setClearOpen(false)} title="Clear workspace"><form onSubmit={clearWorkspace} className="stack gap-16"><p className="muted">This only affects your account. Type <strong>CLEAR</strong> to confirm. Recovery copy will be available in Settings.</p><label className="field"><span>What should be cleared?</span><select value={clearScope} onChange={e=>setClearScope(e.target.value)}><option value="budget">Budget data only</option><option value="workspace">Budget data and Notes</option></select></label><label className="field"><span>Confirmation</span><input required value={clearConfirm} onChange={e=>setClearConfirm(e.target.value)} placeholder="Type CLEAR" autoComplete="off"/></label><div className="modal-actions"><button type="button" className="button ghost" onClick={()=>setClearOpen(false)}>Cancel</button><button className="button danger" disabled={clearConfirm !== 'CLEAR'}>Clear workspace</button></div></form></Modal>
  </div>
}
