...
export async function configureReminder(settings) {
  ...
  if (permission.display !== 'granted') throw new Error('Notifications are blocked. Enable Budgetly notifications in device Settings.')
  const time=settings.reminder_time||'20:00'
  if(quietAt(settings,time)||!reminderBody(settings))return true
  const notifications=Array.from({length:30},(_,i)=>{const day=new Date(Date.now()+i*86400000);return {id:1100+i,title:'Budgetly',body:reminderBody(settings),schedule:{at:new Date(saveDate(dateInput(day).slice(0,10)+'T'+time)),allowWhileIdle:true}}}).filter(n=>n.schedule.at>new Date())
...
export async function testNotification() {
  if (isNativeApp()) {
    ...
    await LocalNotifications.schedule({notifications:[{id:1002,title:'Budgetly',body:'Your device notifications are ready.',schedule:{at:new Date(Date.now()+3000)}}]})
    return
  }
