"""Build the beginner guide and the identical download served by Budgetly."""
...
def footer(canvas, doc):
    ...
    canvas.drawString(18*mm,12*mm,'Budgetly | Bank messages | September 2026')
...
page(1,'Start here','Your bank messages,\none small step at a time',
    ...
    p('A bank alert is the text your bank sends after a purchase. Budgetly stores a copy for you to review. It does not connect to your bank, move money, or fill in the expense amount automatically.'),
...
page(2,'Before you start','A few useful basics',
    p(f'Open Budgetly in your browser: <link href="{SITE}" color="#147568">{SITE}</link>'),
    ...
    fields([('Tap / press and hold','Tap once to open something. Press and hold your finger down to reveal options such as Copy.'),('Copy / paste','Copy temporarily remembers text. Press and hold inside an empty box, then tap Paste to place it there.'),('Pending review','A message has arrived, but no expense has been created and no wallet balance has changed.'),('Wallet','The place in Budgetly where you track money, for example Main wallet. It is not a bank connection.'),('Forwarding key','A private pass for an automation app to add messages to your account. It is not your password or a bank code.'),('Address / URL','The destination to which the helper app sends the message.'),('Automation / flow','A saved set of phone instructions that runs when something happens.')]),
...
page(3,'Android | Easy method','Copy, paste and review',
    ...
    step(3,'Switch to Budgetly. Open <b>Bank messages</b> and tap <b>Add message</b>.'),
...
page(4,'Android | Optional forwarding','Which app should I use?',
    ...
    step(2,'Open Automate. A <b>flow</b> is a diagram of instructions. Create a new flow with the plus button, open its editor, and name it <b>Budgetly - NBK</b>. Start with just one bank.'),
...
page(5,'Android | Optional forwarding','Choose messages and filter them',
    ...
    step(4,'Set Title to <b>Send transaction alert to Budgetly?</b> Set Message to <b>message</b> in formula mode. Keep <b>Show window off</b>; do not grant appear-on-top permission. This uses an ordinary notification you tap to review the text. [A3]'),
...
page(6,'Android | Optional forwarding','Tell Automate where to send it',
    ...
    p('The key and message are sent to Budgetly over HTTPS. You do not need to know what JSON means beyond selecting this format and copying the supplied text.'))
...
page(7,'Android | Optional forwarding','Connect, test and stop',
    ...
    step(3,'Tap the Automate notification, read the sample, and confirm. In Budgetly, tap <b>Refresh</b>. The test should appear as pending. Delete the test with its bin button; <b>do not record it as an expense</b>.'),
    step(4,'Stop the flow, restore your bank\'s exact sender, save, and start it once. Check both Budgetly and your bank app before relying on it.'),
    ...
    p('<b>To stop:</b> open this flow in Automate and tap Stop. Then disable forwarding in Budgetly (page 13). For another bank, duplicate the stopped flow and update its sender and both bank labels.'))
...
page(8,'iPhone | Easy method','Start with copy and paste',
    ...
    step(3,'Switch to your browser and open Budgetly. Sign in, open the menu, choose <b>Bank messages</b>, and tap <b>Add message</b>.'),
    ...
    note('The date initially shown is the current date/time. Change it for an older alert. Budgetly does not automatically extract the amount, date or merchant. Only one-time expenses can be recorded here.'),
    ...
    p('A third-party app calling itself an SMS forwarder does not gain unrestricted access to your iPhone inbox. Some still require Apple Shortcuts and do not send Budgetly\'s required fields. You do not need to buy one for this guide.'),
...
page(9,'iPhone | Optional Shortcuts','Build a one-tap sending helper',
    ...
    step(1,'Create a forwarding key in Budgetly using page 2. Keep it private. Then open <b>Shortcuts</b>, choose the Shortcuts tab and tap <b>+</b> to create a shortcut. Name it <b>Budgetly - NBK</b>.'),
...
page(10,'iPhone | Optional Shortcuts','Fill the fields and try it once',
    ...
    note('<b>For daily use:</b> copy an actual spending alert, run this shortcut, and paste it into the prompt. Then review the expense in Budgetly. This shortcut gives every run a new reference: running it twice for the same alert can create two pending items. Compare them before recording.'),
    p('If you see an error, use page 12. Do not assume "shortcut finished" means Budgetly accepted the message.'))
...
page(11,'iPhone | Optional automation','Run when a bank SMS arrives',
    ...
    step(1,'Duplicate your working shortcut and name it <b>Budgetly - NBK incoming</b>. Keep the original for manual use. In the duplicate, replace Ask for Input with <b>Get Text from Input</b> and select <b>Shortcut Input</b> as its input.'),
...
page(12,'Help | Both phone types','When something does not work',
    fields([('<b>What you see</b>','<b>What to do next</b>'),('No item in Budgetly','Check internet access. Open Bank messages and tap Refresh. Confirm you are signed into the same account that created the key. Check that the helper actually ran.'),('401 / invalid key','Replace the key in Budgetly and update every helper using it. Paste it into X-FlowBudget-Key, not into the address.'),('422 / message rejected','Do not retry a security message. For a genuine spending alert, check bank spelling, all three fields, and that the text is under 2,000 characters. The reference must be nonempty and no longer than 160 characters.'),('429 / too many messages','The account has reached the pending-message limit (500). Review or deliberately remove old pending items first.'),('500, 503 or could not connect','Wait and check Budgetly in your browser. Before resending, check whether the message already arrived. Use the original bank text to add a missing item later.'),('Wrong amount or old date','The review form requires manual entry. Correct it before recording. An already recorded expense must be changed in Transactions.'),('A duplicate appears','Do not record both. Delete the extra pending item after comparing the message, time and amount. iPhone helper retries can have different references.'),('Android helper stopped','Inspect the flow log for a connection error. Start it once after the cause is fixed. This simple flow has no guaranteed offline queue; compare against the original SMS and paste missing alerts.'),('iPhone sender unavailable','Use the manual shortcut or copy/paste. Do not broaden forwarding to all of your messages.'),('Bank app shows a warning','Stop the forwarding helper. Use Budgetly in the browser with password-only sign-in. Record the warning and contact the bank if needed; do not disable bank protection or Play Protect.')]),
...
page(13,'Help | Both phone types','Pause, protect and get help',
    ...
    step(1,'Android: stop the Budgetly flow in Automate. iPhone: open Shortcuts, tap Automation, select the rule and disable it (Enable This Automation off or Don\'t Run, depending on version). [I6]'),
    step(2,'In Budgetly, open Bank messages, find Phone forwarding and tap <b>Disable forwarding</b>. Confirm. Saved messages and recorded expenses remain; future submissions using that key are rejected.'),
    ...
    p('Budgetly\'s own direct SMS capture is disabled. External helpers are optional and independently permissioned. A forwarding key is not a bank login; it cannot move money or read your data. The security-word filter is limited, so you must exclude security messages before sending them.'),
    ...
    p('App availability, screens and payment terms can change. Developer instructions confirm capabilities, not a physical-device compatibility certification. These are independent apps, not Budgetly partners.','SmallFB'),
...
if __name__ == '__main__':
    ...
    doc=SimpleDocTemplate(str(OUT),pagesize=A4,rightMargin=18*mm,leftMargin=18*mm,topMargin=16*mm,bottomMargin=24*mm,title='Budgetly - Bank Messages for Beginners',author='Budgetly')
